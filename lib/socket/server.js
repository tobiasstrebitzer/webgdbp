...
(function(exports) {
   ...
   */
  function SocketServer(addr, port, options) {
    this.addr = addr;
    this.port = port;
   ...
   * preferred network as the addr parameter on SocketServer contructor.
   */
  SocketServer.getNetworkAddresses=function(callback) {
   ...
   */
  SocketServer.prototype.listen = function(onAcceptCallback, onDisconnectCallback, onDataReadCallback) {
    // Register connect callback.
    this.callbacks.connect = onAcceptCallback;
    this.callbacks.disconnect = onDisconnectCallback;
    this.callbacks.recv = onDataReadCallback;
  ...
  SocketServer.prototype.disconnect = function() {
    ...
    if (this.callbacks.disconnect) {
      this.callbacks.disconnect(this.serverSocketId);
    }
   ...
   */
  SocketServer.prototype._onCreate = function(createInfo) {
   ...
   * @private
   */
  SocketServer.prototype._onListenComplete = function(resultCode) {
    if (resultCode===0) {
  ...
  SocketServer.prototype._onAccept = function(resultInfo) {
    ...
    if (resultInfo.resultCode===0) {
      ...
      var tcpConnection = new SocketConnection(resultInfo.socketId);
      tcpConnection.addDataReceivedListener(this.callbacks.recv);
  ...
  SocketServer.prototype._onNoMoreConnectionsAvailable = function(socketId) {
  ...
  SocketServer.prototype._onSocketInfo = function(tcpConnection, socketInfo) {
   ...
   */
  function SocketConnection(socketId) {
  ...
  SocketConnection.prototype.setSocketInfo = function(socketInfo) {
    this.socketInfo = socketInfo;
  };

  SocketConnection.prototype.requestSocketInfo = function(callback) {
   ...
   */
  SocketConnection.prototype.addDataReceivedListener = function(callback) {
   ...
   */
  SocketConnection.prototype.sendMessage = function(msg, callback) {
    _stringToArrayBuffer(msg + '\00', function(arrayBuffer) {
   ...
   * @private
   */
  SocketConnection.prototype._onSocketInfo = function(callback, socketInfo) {
   ...
   *
   * @private
   * @see SocketConnection.prototype.addDataReceivedListener
   * @param {Object} readInfo The incoming message
   */
  SocketConnection.prototype._onDataRead = function(readInfo) {
   ...
   */
  SocketConnection.prototype._onWriteComplete = function(writeInfo) {
    // Call sent callback.
